...
frappe.ui.form.on("Purchase Invoice", {
	...
	porcentaje_de_retencion_renta: function (frm) {
		frm.events.calcular_total_retencion(frm);
	},

	porcentaje_de_retencion_iva: function (frm) {
		frm.events.calcular_total_retencion(frm);
	},
	...
	tipo_retencion_renta: function (frm) {
		...
		frappe.call({
			method: "sri.sri.doctype.agentes_de_retencion.agentes_de_retencion.get_porcentajesRenta",
			...
			callback: function (r) {
				...
				if (opciones.length === 1) {
					frm.set_value("porcentaje_de_retencion_renta", opciones[0]);
	...
	tipo_retencion_iva: function (frm) {
		const tipo = frm.doc.tipo_retencion_iva;

		frm.set_df_property("porcentaje_de_retencion_iva", "read_only", !tipo);

		if (!tipo) {
			frm.set_df_property("porcentaje_de_retencion_iva", "options", []);
			frm.set_value("porcentaje_de_retencion_iva", null);
			frm.refresh_field("porcentaje_de_retencion_iva");
			return;
		}

		frappe.call({
			method: "sri.sri.doctype.agentes_de_retencion.agentes_de_retencion.get_porcentajesIva",
			args: { tipo_retencion: tipo },
			callback: function (r) {
				frm.set_df_property("porcentaje_de_retencion_iva", "options", r.message || []);

				if (r.message && r.message.length === 1) {
					frm.set_value("porcentaje_de_retencion_iva", r.message[0]);
				}
	...
	calcular_total_retencion: function (frm) {
		...
		if (base_renta < 0) base_renta = 0;

		let valor_retenido_renta = base_renta * (porc_renta / 100);

		let valor_no_retenido_renta = base_renta - valor_retenido_renta;

		let base_total_iva = flt(frm.doc.total_taxes_and_charges);

		let valor_retenido_iva = base_total_iva * (porc_iva / 100);

		let valor_no_retenido_iva = base_total_iva - valor_retenido_iva;

		let total_retener = valor_retenido_renta + valor_retenido_iva;
		let total_no_retener = valor_no_retenido_renta + valor_no_retenido_iva;

		frm.doc.valor_retenido_renta = valor_retenido_renta;
		frm.doc.valor_no_retenido_renta = valor_no_retenido_renta;
		...
		frm.refresh_field("valor_no_retenido_renta");
		frm.refresh_field("valor_retenido_iva");
